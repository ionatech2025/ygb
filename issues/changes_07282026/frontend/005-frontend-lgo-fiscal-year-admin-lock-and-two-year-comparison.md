...
- [ ] Component test: two FY blocks rendered with (a)/(b) labels.
- [ ] Component test: young men count field present per FY block.
- [ ] Component test: parish labels distinguish "in the district" vs "received PDM funds".
- [ ] Component test: collector cannot change admin-selected current FY.
- [ ] Admin component test: set fiscal year calls PUT API and shows success.
- [ ] Unit test: payload contains two `fiscalYearRecords` with correct labels.
