...
|----------|---------|------------------------|
| `PdmEntryScreen.tsx` | Respondent category (BYP / IYP / LGO / PC) | Native `<select>` — worst offender on mobile |
| `RespondentSection.tsx` | Gender, age group | Native `<select>` |
| `CascadingLocationSelector.tsx` | District, sub-county, parish, village | Native `<select>` × 4 |
| Form-specific sections | Various enums (e.g. BYP fund section, rating selects) | Native `<select>` — audit during implementation |
...
- [ ] Component test: `FormSelect` renders options, fires `onChange`, exposes accessible name/label.
- [ ] Component test: long option labels wrap without horizontal overflow on 320px viewport.
- [ ] `PdmEntryScreen.test.tsx`: selecting BYP / IYP / LGO / PC still loads the correct form.
- [ ] `RespondentSection` / location tests: gender, age group, and cascade selects still work with Testing Library `selectOptions` equivalent (click/tap option).
...
- [ ] No regression on desktop collector forms.
