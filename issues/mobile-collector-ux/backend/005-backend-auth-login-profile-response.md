...
- [x] **Application test** (`AuthenticateUserServiceTest`): result includes `fullName` from persisted user.
- [x] **Adapter test** (`AuthControllerTest`): JSON body contains `user.fullName` matching seeded collector/admin.
- [x] **Domain / VO tests** — none required beyond existing user model.
- [x] OpenAPI / DTO validation: `AuthResponse` documents new fields.
- [x] No business logic in controller or DTO classes.

## Implementation notes

- `AuthenticatedUserProfile` application record; `AuthenticationResult(token, user)`.
- `AuthUserResponse` REST DTO; `AuthResponse(token, user)`.
- `AuthMapper.toResponse()` maps profile via MapStruct.
