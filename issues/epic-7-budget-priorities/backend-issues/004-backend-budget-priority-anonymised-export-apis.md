...
- [ ] Write **Domain Tests** for export column whitelist — no PII column names in schema.
- [ ] Write **Application Tests** with mocked read port + export generator — correct format invoked with projected rows.
- [ ] Write **Controller Tests**:
  - `GET .../download/csv` → `200`, `Content-Type: text/csv`.
  - `GET .../download/excel` → `200`, spreadsheet content type.
  - Unauthenticated access allowed.
- [ ] Write **Integration Test**: seed data → download → parse file → row count matches filter; no phone column present.
- [ ] Implement use case, endpoints, mappers, streaming export.
