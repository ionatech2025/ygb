...
- [x] Write **Domain Tests** for `BudgetPriorityDashboardFilter` validation.
- [x] Write **Domain Tests** for anonymised projection — sample row with PII → public DTO has no PII fields.
- [x] Write **Application Tests** with mocked read port — summary and chart queries pass filter correctly.
...
- [x] Write **Integration Test** with seeded submissions: summary totals match DB counts.
- [x] Implement filter model, read port, services, REST endpoints, Flyway indexes if needed.
