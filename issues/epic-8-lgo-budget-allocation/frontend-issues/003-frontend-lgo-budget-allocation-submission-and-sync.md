...
- [x] Unit test: enqueue payload includes `deviceSubmissionId` and form discriminator.
- [x] Component test: submit enqueues locally and shows “Saved locally” when offline (mock queue).
- [x] Component test: online submit triggers sync store `triggerSync`.
- [x] Integration test (or sync engine unit test): queued item maps to correct API endpoint.
- [ ] Manual test checklist: airplane mode → submit → item pending → online → synced.
- [x] Implement submit wiring, success UI, and sync engine route registration.
