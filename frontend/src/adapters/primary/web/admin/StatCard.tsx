...
export function StatCard({ card, accentIndex = 0, className = '' }: StatCardProps) {
  ...
  return (
    <article
      ...
      className={`${adminDashboardClasses.statCard} ${accent.ring} ${className}`.trim()}
    >
      <div className="flex items-start justify-between gap-3">
        <h4 className={`${adminDashboardClasses.statCardTitle} min-w-0 flex-1`}>{card.title}</h4>
