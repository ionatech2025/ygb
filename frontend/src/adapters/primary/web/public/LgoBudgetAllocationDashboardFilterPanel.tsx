...
import { hasActiveLgoBudgetAllocationDashboardFilters } from '../../../../core/domain/lgo-budget-allocation-dashboard-filter.model';
...
import { LgoBudgetAllocationScalarFilters } from './LgoBudgetAllocationScalarFilters';

export interface LgoBudgetAllocationDashboardFilterPanelProps {
  dashboardApi: ILgoBudgetAllocationDashboardApiPort;
  locationRepository?: ILocationRepositoryPort;
}

export function LgoBudgetAllocationDashboardFilterPanel({
  dashboardApi,
  locationRepository: locationRepo = locationRepository,
}: LgoBudgetAllocationDashboardFilterPanelProps) {
  const filter = useLgoBudgetAllocationDashboardFilterStore((state) => state.filter);
  const locationFilterError = useLgoBudgetAllocationDashboardFilterStore((state) => state.locationFilterError);
  const setFilter = useLgoBudgetAllocationDashboardFilterStore((state) => state.setFilter);
  const setLocationFilterError = useLgoBudgetAllocationDashboardFilterStore((state) => state.setLocationFilterError);
  const clearAll = useLgoBudgetAllocationDashboardFilterStore((state) => state.clearAll);
  ...
  const active = hasActiveLgoBudgetAllocationDashboardFilters(filter);
      ...
      <div className={publicDashboardClasses.panelHeader}>
        ...
        {active && (
          <button
            type="button"
            onClick={clearAll}
            data-testid="lgo-dashboard-filter-clear-all"
      ...
      {expanded && (
        <div className={`space-y-5 ${publicDashboardClasses.panelInset}`}>
          {locationFilterError && (
            <div
              role="alert"
              data-testid="lgo-dashboard-location-filter-error"
          ...
          <div data-testid="lgo-filter-location">
          ...
          <LgoBudgetAllocationScalarFilters
            filter={filter}
