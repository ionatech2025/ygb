...
@Repository
class PublicAnonymisedExportJpaRepository {

    @PersistenceContext
    private EntityManager entityManager;

    PublicAnonymisedRecordPage findRecords(DashboardFilter filter, PageRequest pageRequest) {
        Map<String, Object> params = new java.util.HashMap<>();
        String whereClause = DashboardFilterSqlSupport.whereClause(filter, params);
        long totalElements = countRecords(whereClause, params);

        String sql = """
                SELECT s.id, s.form_type, s.district_id, d.name,
                       s.respondent_gender, s.respondent_age_group,
                       s.form_completed_at, s.status, s.financial_year_period
                FROM submissions s
        ...
        List<PublicAnonymisedRecord> items = rows.stream().map(this::mapRecordRow).toList();
        return new PublicAnonymisedRecordPage(items, totalElements, pageRequest.page(), pageRequest.size());
    }

    private long countRecords(String whereClause, Map<String, Object> params) {
    ...
    private PublicAnonymisedRecord mapRecordRow(Object[] row) {
